...
class RedBlackTree:
    ...
    def _remove_this_node(self) -> None:
        # Remove the tree node as specified by "self"
        if self.left and self.right:
            ...
            if self.color == 1:
                # This node is red
                self._remove_red(child)
            else:
                # The node is black
                self._remove_black(child)
    
    def _remove_red(self, child) -> None:
        # The node to remove is red, and its child is black
    ...
    def _remove_black(self, child) -> None:
        # The node to remove is black
        if child is None:
            ...
            # The node to remove is black and its child is red
